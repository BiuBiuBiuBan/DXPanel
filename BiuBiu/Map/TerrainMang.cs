...
public class TerrainMang : MonoBehaviour
{
    ...
    private TerrainTile[,] mapTileList;
    public void Init(int row, int col)
    {
        //row = 5;
        //col = 5;
        map = GameObject.Find("OBJ");
        mapTile = Resources.Load<GameObject>("Terrainpre");
        this.row = row;
        this.col = col;
        mapTileList = new TerrainTile[row, col];
        initMap();
    }
    ...
    private void initMap()
    {
        for (int i = 0; i < row; i++)
        {
            for (int j = 0; j < col; j++)
            {
                ...
                TerrainTile tile = tmp.GetComponent<TerrainTile>();
                tile.setXY(i, j);
    ...
    void Update()
    {
        ...
        {
            TerrainTile tile = getGameXY(Input.mousePosition);
            tile.GetComponent<SpriteRenderer>().sprite = AssetDatabase.LoadAssetAtPath<Sprite>("Assets/Image/Image/building_bianli_T.png");
    ...
    /// <returns></returns>
    public TerrainTile getGameXY(Vector2 pos)
    {
        ...
        //根据矩形算出来的图快
        TerrainTile tile = mapTileList[x, y];
